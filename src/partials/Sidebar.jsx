          ...
          <NavLink end to="/" className="block">
            <svg width="32" height="32" viewBox="0 0 32 32">
              ...
              <path
                ...
                fill="url(#logo-b)"
              />
            </svg><div>v{process.env.REACT_APP_VERSION}</div>
          </NavLink>
        </div>
        ...
        <div className="space-y-8">
          <div>
            <ul className="mt-3">
              ...
              {
                sidebarSettings ?
                Object.keys(sidebarSettings).map((sectionName)=> {                  
                  {
                    return ([
                        <div key={sectionName}>
                          <h3 className="text-xs uppercase text-slate-500 font-semibold pl-3">
                            <span className="lg:hidden lg:sidebar-expanded:block 2xl:block">{sectionName}</span>
                          </h3>
                          <ul className="mt-3">
                          </ul>
                        </div>,
                        Object.keys(sidebarSettings[sectionName]).map((sidebarPageGroupName)=> {
                          const pageGroupLinks = Object.entries(sidebarSettings[sectionName][sidebarPageGroupName].links);
                          const pageGroupSVGPaths = Object.entries(sidebarSettings[sectionName][sidebarPageGroupName].svg.paths);
                          return (
                                    <SidebarPageGroup
                                    key={sidebarPageGroupName}
                                    pathname={pathname} 
                                    sectionLinks={pageGroupLinks}
                                    sectionName={sidebarPageGroupName}
                                    sectionSVGPaths={pageGroupSVGPaths}
